import { keydown, wheel, wheelSupported } from './helper'
  ...
  describe('basic', () => {
    ...
    describe('close modal', () => {
      ...
      it('closes modal by clicking wrapper area', () => {
        $container.find('.lb-wrapper').click();
        expect($('body').find('.lb-modal').length).toEqual(0);
      });

      it('does not close by clicking content area', () => {
        $container.find('.lb-content').click();
    ...
    describe('new window', () => {
      it('opens new window', () => {
        spyOn(window, 'open');
        $container.find('.lb-window').click();
    ...
    describe('zoom', () => {
      it('zooms by click', () => {
        ...
        expect($container.hasClass('lb-zooming')).toEqual(true);
        $container.find('.lb-zoom').click();
        expect($container.hasClass('lb-zooming')).toEqual(false);
      });
    ...
    describe('navi', () => {
      it('moves by click', () => {
        $container.find('.lb-next').click().click().click();
        expect($links.eq(2).hasClass('lb-current')).toEqual(true);

        $container.find('.lb-prev').click().click().click();
        expect($links.eq(0).hasClass('lb-current')).toEqual(true);
      });
